...
`query-auth.enabled = true`, `scan.ignore-corrupt-files = true`, and
`scan.ignore-lost-files = true`.
...
`servedGeneration`, `servedSnapshotId`, and snapshot identity. The existing `getValues` method is a
...
fingerprint, served snapshot ID, snapshot identity, generation, and one address and server epoch per
shard. This release predates `Snapshot.uuid()`, so the wire field named `snapshotUuid` contains
`sha256:<hex>` of the exact snapshot-file bytes. The monitor parses the same bytes that it hashes and
propagates that identity through bootstrap to every executor. Reusing a snapshot ID with different
bytes therefore advances the generation fence. Legacy descriptors without an identity are
...
bootstrap subtasks. Lookup access to the release-native local RocksDB state is serialized per
